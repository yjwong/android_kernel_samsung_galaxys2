...
int fimc_reqbufs_capture(void *fh, struct v4l2_requestbuffers *b)
{
	...
	if (cap->fmt.priv == V4L2_PIX_FMT_MODE_CAPTURE && b->count == 1)
		ctrl->mem.cap_curr = PAGE_ALIGN(ctrl->mem.curr);
	else
		ctrl->mem.cap_curr = 0;
	...
	mutex_unlock(&ctrl->v4l2_lock);
	for (i = 0 ; i < b->count ; i++)
		printk(KERN_INFO "alloc base = 0x%x\n",
			ctrl->cap->bufs[i].base[FIMC_ADDR_Y]);
	return 0;
}
		...
		if (ctrl->id != FIMC2) {
			...
			if (ret < 0) {
				...
				if ((ctrl->cap->vt_mode == 1) &&
					...
					dev_err(ctrl->dev, "vtmode = 1, rotate = %d,"
						" cam->width = %d,"
						" cam->height = %d\n", cap->rotate,
			...
			if (cam->type == CAM_TYPE_MIPI) {
				...
				*/
				...
				if (fimc->active_camera == 0) {
#if (0) /* in case of ISP on Back camera.*/
