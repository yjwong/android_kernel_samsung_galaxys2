...
{
	spin_lock(&info->power_lock);
#ifndef CONFIG_EXYNOS4_FPGA
	clk_enable(info->clock);
	fimg2d_debug("clock enable\n");
#endif
#ifdef CONFIG_S5P_SYSMMU_FIMG2D
	sysmmu_on(SYSMMU_G2D, 0);
...
static int fimg2d_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "FIMG2D clock(name:%s, rate:%ld) parent(name:%s, rate:%ld)\n",
			pdata->clkname, clk_get_rate(sclk), pdata->parent_clkname, clk_get_rate(parent));
