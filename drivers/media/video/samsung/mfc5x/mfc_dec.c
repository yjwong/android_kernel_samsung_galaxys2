...
int mfc_init_decoding(struct mfc_inst_ctx *ctx, union mfc_args *args)
{
...
#if 0
#if defined(CONFIG_CPU_FREQ) && defined(EXYNOS4_VA_PMU)
	...
	if (ctx->width >= 1280 || ctx->height >= 720) {
		if (atomic_read(&ctx->dev->cpufreq_lock_cnt) == 0) {
			...
			s5pv310_cpufreq_lock(DVFS_LOCK_ID_MFC, cpu_lv); /* 500MHz */
			mfc_info("[%s] CPU Freq Locked 500MHz!!\n", __func__);
		}
...
int mfc_check_resolution_change(struct mfc_inst_ctx *ctx, struct mfc_dec_exe_arg *exe_arg)
{
	...
	resol_status =
		(read_reg(MFC_SI_DISPLAY_STATUS) & (DISP_RC_MASK << DISP_RC_SHIFT)) & DISP_RC_MASK;
